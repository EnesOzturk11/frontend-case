...
export function SightingsCardContent({ checkin }: Readonly<{ checkin: SightingsCardData }>) {
  const { personName, personAvatar, seenWith, timestamp, location, coordinates, note } = checkin

  return (
    <div className="flex flex-col overflow-hidden rounded-2xl bg-(--surface) text-(--sea-ink) ring-1 ring-(--chip-line)">
      {/* Header — avatar | reporter + coords */}
      ...
      <div className="flex flex-col gap-3 px-4 py-4">
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-(--sea-ink)">{seenWith}</p>
          </div>
        </div>
...
function PinButton({ label }: Readonly<{ label: string }>) {
  return (
    <button
      type="button"
      aria-label={`View sighting by ${label}`}
    ...
    >
      <span className="absolute top-0 flex h-7 w-7 items-center justify-center rounded-full bg-(--lagoon) shadow-lg shadow-(--lagoon)/40 ring-2 ring-(--foam) transition-transform duration-150 group-hover:-translate-y-1 group-focus:-translate-y-1">
        <span className="material-symbols-outlined text-[15px] text-(--foam)">visibility</span>
      </span>
...
export function SightingsCard({
  ...
  return (
    <Popover>
      ...
      >
        <SightingsCardContent checkin={checkin} />
      </PopoverContent>
    </Popover>
  )
}